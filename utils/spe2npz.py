...
def convert(files):
    ...
    for f in files:
        info = grab_info(f)
        x_uncal,x, y = make_data_not_suck(info)
        livetime, totaltime, starttime = make_info_not_suck(info)
        np.savez(f.split(spe)[0], info=info, x_uncal=x_uncal, x=x, y=y, livetime=livetime,
...
def make_data_not_suck(webster):
    ...
    xaxis_uncal = np.arange(e_range[0], e_range[1]+1, 1)
    correction = (webster['MCA_CAL:'].split('\n'))[1]
    cfunc = lambda x, k: k[0] + x*k[1] + x*x*k[2]
    k = correction.split()[0:3]
    k = [ float(v) for v in k[:] ]
    xaxis = [ cfunc(v, k) for v in xaxis_uncal[:] ]
    
    return xaxis_uncal,xaxis, yaxis    
